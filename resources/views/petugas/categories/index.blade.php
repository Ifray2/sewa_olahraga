            ...
            <div class="table-card-actions">
                ...
                <form method="GET" style="display:flex;gap:.5rem;flex-wrap:wrap;">
                    ...
                    @if(request('search') || request('status') !== null)
                        <a href="{{ route('petugas.categories.index') }}" class="btn-outline">Reset</a>
                    @endif
                </form>
                <a href="{{ route('petugas.categories.create') }}" class="btn-add">+ Tambah Kategori</a>
            </div>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-group">
                                <a href="{{ route('petugas.categories.show', $category) }}" class="btn-action btn-view">Lihat</a>
                                <a href="{{ route('petugas.categories.edit', $category) }}" class="btn-action btn-edit">Edit</a>
                                <form method="POST" action="{{ route('petugas.categories.toggleStatus', $category) }}" style="display:inline;">
                                    @csrf @method('PATCH')
                                ...
                                <form method="POST" action="{{ route('petugas.categories.destroy', $category) }}" style="display:inline;"
                ...
                @empty
                    <tr>
                        <td colspan="7" style="text-align:center;padding:3rem;color:var(--mid);">
                            ...
                            <a href="{{ route('petugas.categories.create') }}" class="btn-add" style="display:inline-flex;margin-top:1rem;">+ Tambah Kategori</a>
                        </td>
                    </tr>
                @endforelse
            </tbody>
