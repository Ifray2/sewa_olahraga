...
@section('content')
    ...
    <div style="display:flex;gap:1px;background:var(--border);border:1px solid var(--border);border-radius:4px;overflow:hidden;margin-bottom:1.5rem;">
        ...
        @foreach($tabs as $val => $tab)
            <a href="{{ route('petugas.rentals.index', array_merge(request()->except('page'), ['status' => $val ?: null])) }}"
            ...
            <div class="table-card-actions">
                <form method="GET" style="display:flex;gap:.5rem;flex-wrap:wrap;align-items:center;">
                    ...
                    @if(request('search') || request('date_from') || request('date_to'))
                        <a href="{{ route('petugas.rentals.index', request('status') ? ['status' => request('status')] : []) }}" class="btn-outline">Reset</a>
                    @endif
                </form>
                <a href="{{ route('petugas.rentals.create') }}" class="btn-add">+ Buat Transaksi</a>
            </div>
        </div>
        ...
        <table class="data-table">
            ...
            <tbody>
                @forelse($rentals as $rental)
                    <tr>
                        <td>
                            <a href="{{ route('petugas.rentals.show', $rental) }}"
                        ...
                        <td>
                            <div class="action-group" style="flex-wrap:wrap;">
                                <a href="{{ route('petugas.rentals.show', $rental) }}" class="btn-action btn-view">Lihat</a>

                                @if($rental->status === 'pending')
                                    <form method="POST" action="{{ route('petugas.rentals.confirm', $rental) }}" style="display:inline;">
                                        @csrf
                                ...
                                @if($rental->status === 'confirmed')
                                    <form method="POST" action="{{ route('petugas.rentals.activate', $rental) }}" style="display:inline;">
                                        @csrf
                                        <button type="submit" class="btn-action"
                                ...
                                @if($rental->status === 'active')
                                    <form method="POST" action="{{ route('petugas.rentals.return', $rental) }}" style="display:inline;"
                                ...
                                @if(!in_array($rental->status, ['returned','cancelled']))
                                    <form method="POST" action="{{ route('petugas.rentals.cancel', $rental) }}" style="display:inline;"
