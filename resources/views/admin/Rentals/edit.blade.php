<x-app-layout>
    <x-slot name="heading">Edit Transaksi</x-slot>
    <x-slot name="breadcrumb">
        <a href="{{ route('dashboard') }}">Admin</a>
        <span>/</span>
        <a href="{{ route('admin.rentals.index') }}">Transaksi</a>
        <span>/</span>
        <a href="{{ route('admin.rentals.show', $rental) }}">{{ $rental->rental_code }}</a>
        <span>/</span>
        <span>Edit</span>
    </x-slot>
    ...
    <form method="POST" action="{{ route('admin.rentals.update', $rental) }}" id="edit-form">
        @csrf @method('PUT')
            ...
            <div style="display:flex;flex-direction:column;gap:1.5rem;position:sticky;top:70px;">
                ...
                <div class="table-card">
                    ...
                    <div style="padding:1rem 1.25rem;border-top:1px solid var(--border);display:flex;flex-direction:column;gap:.5rem;">
                        ...
                        <a href="{{ route('admin.rentals.show', $rental) }}" class="btn-outline" style="justify-content:center;padding:.65rem;">
                            Batal
                        </a>
                    </div>
                </div>
